...
2. 도입부 (공감 가는 일상적 상황)
3. 원인/설명 (쉬운 말로)
4. 관리 방법 (구체적 수치 포함, 단계별 리스트)
5. 주의사항
6. FAQ (Q/A 형식, 3개)
7. 마무리
8. 핵심 요약 (2~3문장, AI 인용용)
...
def review_and_refine_post(keyword_data, initial_content):
    """생성된 블로그 글을 3단계 기준으로 검토 후 최종본 반환"""

    prompt = f"""아래는 '{keyword_data["keyword"]}'를 주제로 작성된 치과 블로그 초안이야.
다음 3단계 기준으로 검토하고 최종 완성본만 반환해줘.
...
1단계 - 병원 마케팅 관점 검토:
- 과장·허위 표현, 의료광고법 위반 소지 제거
- 환자가 치과를 방문하고 싶어지는 동기 부여가 되는지 확인
- 치료 필요성이 자연스럽게 느껴지는지 확인

2단계 - 네이버 블로그 구조 최적화:
- 제목: 검색에 잘 걸리고 클릭하고 싶은 제목인지 확인
- 소제목: [ ] 형식 유지, 흐름이 자연스러운지 확인
- 본문: 문단이 너무 길지 않은지, 리스트가 적절히 활용됐는지 확인

3단계 - 환자 친화적 말투 검토:
- 어려운 의학 용어는 괄호 안에 쉬운 설명 추가
- 딱딱한 문장은 자연스럽고 따뜻한 말투로 수정
- 읽는 사람이 공감하고 끝까지 읽고 싶어지는 흐름인지 확인

[출력 규칙]
- 검토 과정·코멘트 없이 최종 완성된 블로그 글만 반환
- 원래 구조(도입부 → 원인 → 관리법 → 주의사항 → FAQ → 마무리 → 핵심 요약) 유지
- 공백 제외 1200자 이상 유지
"""
    ...
    response = client.messages.create(
        ...
        system="너는 의료광고법을 준수하는 치과 병원 마케팅 전문가이자 블로그 에디터다."
    )

    refined = response.content[0].text
    print("[검토 완료] 3단계 검토 및 정제 완료")
    return refined
...
def run_blog_generation():
    ...
    refined_content = review_and_refine_post(keyword_data, content)

    filtered_content = apply_medical_filter(refined_content)
